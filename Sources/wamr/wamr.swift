        ...
        do {
            let module = try wasmThrowsfy { (errorBuffer, errorBufferSize) in
                binary.withUnsafeBufferPointer { binary in
                    wasm_runtime_load(
                        UnsafeMutablePointer(mutating: binary.baseAddress!), UInt32(binary.count),
...
internal func wasmThrowsfy<T>(
    _ fn: (_ errorBuffer: UnsafeMutablePointer<UInt8>,
    ...
    guard let value = result else {
        throw WasmRuntimeError(
            message: String(decoding: buffer[0..<strnlen(buffer, 1024)], as: UTF8.self)
        )
    }
    return value
}
